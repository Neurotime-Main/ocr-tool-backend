...
const SECRET = new Set([
  'SERVER_DB_HOST', 'SERVER_DB_USER', 'SERVER_DB_PASSWORD',
...
const template = [
  ...
  '# Optional secret for POST /api/ocr/image. Scrapers send it as X-API-Key.',
  `OCR_SERVICE_API_KEY=${quoted('OCR_SERVICE_API_KEY')}`,
  section('Workspace'),
  '# There is no application database. Uploads, recognised pages and highlights',
  "# live in the API process's memory and are gone when it restarts, so a batch",
  '# has to be published in the same run it was uploaded in.',
  '#',
  '# This caps how many documents are kept before the oldest finished ones are',
  '# dropped, taking their stored PDFs with them. It is what bounds the heap.',
  `# OCR_MAX_RETAINED_DOCUMENTS=${value('OCR_MAX_RETAINED_DOCUMENTS', '200')}`,
  ...
  `MAX_UPLOAD_MB=${value('MAX_UPLOAD_MB', '250')}`,
  ...
  '',
  '# How much garbled text a PDF may carry before its own text layer is thrown',
  '# away and the page is recognised from the raster instead. Some issues embed',
  '# fonts whose character map is wrong, so the page reads perfectly while the',
  '# text behind it is nonsense.',
  '#',
  '# The first is the share of suspect words that condemns one page; the second',
  '# is the share of condemned pages that condemns the rest of the document.',
  '# Lower means more accurate text and much more recognition time. Leave them',
  '# blank to use these defaults -- blank is read as "unset", not as zero.',
  `OCR_BROKEN_ENCODING_PAGE=${value('OCR_BROKEN_ENCODING_PAGE', '0.02')}`,
  `OCR_BROKEN_ENCODING_DOCUMENT=${value('OCR_BROKEN_ENCODING_DOCUMENT', '0.5')}`,
  section('Optional'),
  '# Defaults shown. Uncomment only to override.',
  '',
  '# Recognition always runs inside the API process: the queue is that',
  "# process's memory, so a worker anywhere else would have nothing to read.",
  '# RUN_OCR_IN_API is no longer honoured and src/worker.ts refuses to start.',
  ...
  '',
  '# Uploaded photos and scans go directly to OCR as high-quality 4:4:4 JPEG.',
  '# A picture larger than IMAGE_MAX_EDGE on its longest side is scaled down:',
  '# past this, extra pixels cost recognition time without finding more text.',
  '# IMAGE_MAX_EDGE=4000',
  '# IMAGE_JPEG_QUALITY=94',
  '',
  '# Video frames go directly to OCR at one frame per second. A video whose',
  '# fixed-rate frame count exceeds VIDEO_MAX_FRAMES is rejected clearly.',
  '# FFMPEG_BIN=ffmpeg',
  '# FFPROBE_BIN=ffprobe',
  '# VIDEO_FRAME_INTERVAL_SECONDS=1',
  '# VIDEO_MAX_FRAMES=3600',
  '# VIDEO_MAX_EDGE=1920',
  '# VIDEO_CONVERSION_CONCURRENCY=1',
  '# VIDEO_CONVERSION_TIMEOUT_MS=1800000',
  '',
